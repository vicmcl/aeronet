...
class CFDDataLoader:
    def __init__(
        self, vtu_file: str, vtp_file: Optional[str] = None, tolerance: float = 1e-10
    ):
        """Initialize CFD data loader with fluid and surface mesh files."""
        self.vtu_file = vtu_file
        self.vtp_file = vtp_file
        self.tolerance = tolerance

    def load_fluid_data(self) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        """Load fluid mesh data and features from VTU file."""
    ...
    def load_surface_data(self) -> Optional[np.ndarray]:
    ...
    def _combine_points_and_features(
        ...
        """Combine fluid and surface point data with indicators."""
    ...
    def _remove_duplicates(self, data_tensor: torch.Tensor) -> torch.Tensor:
        """Remove duplicate fluid points that overlap with surface points."""
        ...
        print(f"Removed {np.sum(~unique_points_mask)} duplicate fluid points")
...
if __name__ == "__main__":
    ...
    loader = CFDDataLoader(vtu_file, vtp_file)
    ...
    print("Data example:", data_tensor[0])
